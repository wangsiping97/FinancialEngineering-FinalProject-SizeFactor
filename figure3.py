...
df3 = pd.DataFrame({'month': np.arange(0, np.array(size).shape[0] - 60), 'kurt': kurts})

# Draw Plot
plt.figure(figsize=(16, 10), dpi=80)
plt.plot('month', 'kurt', data=df3, color='tab:red')

# Decoration
plt.ylim(0, 1000)
xtick_location = df3.index.tolist()[::12]
xtick_labels = [(x // 12) + 2005 for x in df3.month.tolist()[::12]]
...
plt.gca().set(xlabel='Year', ylabel='Monthly Kurt of Size')
