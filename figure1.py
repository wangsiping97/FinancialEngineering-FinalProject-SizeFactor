...
df1 = pd.DataFrame({'month': np.arange(0, np.array(size).shape[0] - 60), 'ave_size': ave_size})

# Draw Plot
plt.figure(figsize=(16, 10), dpi=80)
plt.plot('month', 'ave_size', data=df1, color='tab:red')

# Decoration
plt.ylim(0, 3000000)
xtick_location = df1.index.tolist()[::12]
xtick_labels = [(x // 12) + 2005 for x in df1.month.tolist()[::12]]
...
plt.gca().set(xlabel='Year', ylabel='Monthly Average of Size')
